...
class ShardWorkerContext implements WorkerContext {
  ...
  /**
   * Apply cgroups limitation wrapper first, before any privilege-dropping wrappers.
   * This is required for cgroupsv2 where the wrapper needs to write to cgroup.procs as root.
   * This method MUST be called before any prioritized execution policies that drop privileges.
   */
  @Override
  ...
  public IOResource limitExecution(
    ...
    IOResource resource = cgroupResource != null ? cgroupResource : new IOResource() {
      @Override
      public void close() {}
